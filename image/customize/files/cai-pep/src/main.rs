...
fn handle_intent(
    ...
    let canonical_workdir = canonicalize_existing_dir(&request.params.params.workdir).map_err(
        |err| PepError::PolicyDeny {
...
fn ensure_command_policy(command: &str, config: &PepConfig, audit_id: &str) -> Result<(), PepError> {
