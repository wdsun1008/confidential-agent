...
2. **Adapt**
   ...
   - Service Surface Proof: before writing the final chat probe, confirm the startup command, dependency closure, listen host/port, and chat endpoint trace to the same upstream service mode or bridge chain. If the chosen mode depends on optional extras, plugins, or adapters beyond the default install, install them explicitly and include an import or command check in the install script.
   - If multiple ports or services exist, such as dashboard, metrics, health, and API listeners, choose the port whose running process also serves the declared chat endpoint. Do not assemble `ExecStart`, `service.connect`, and `verification.json.chat_path` from unrelated files or services.
...
```
   ...
   - If connect succeeds but the chat probe fails, do not blind-retry random endpoints. Return to the Service Surface Proof: check service logs, confirm the `ExecStart` process listens on the declared port, verify optional dependency checks, and confirm the chat endpoint exists on that listener before rebuilding the inconsistent artifact.
